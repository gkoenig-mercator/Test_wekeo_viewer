...
def keep_alive(
    page: Page, browser: Browser, config: Config, storage: Storage, start_time: float
):
    storage.log(
    ...
    while True:
        ...
        if now - last_mouse >= config.mouse_interval:
            ...
            storage.log(
                f"Mouse → ({mouse_x}, {mouse_y})  —  {minutes_formatting_for_elapse_time(storage.session.elapsed())} elapsed"
            )
            last_mouse = now
        ...
        if now - last_check >= config.check_interval:
            if not is_session_alive(page):
                ...
                sys.exit(0)
            storage.log(
                f"Session alive — {minutes_formatting_for_elapse_time(storage.session.elapsed())} elapsed."
            )
            last_check = now
